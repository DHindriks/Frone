...
{
    public Colortypes playerColor;
    public DroneSettings CurrentSelected;
    ...
    private static bool GetLocalMouse( GameObject go, out Vector2 result ) 
    {
        ...
        Vector2 LocalCursor;
        var mp = RectTransformUtility.ScreenPointToLocalPointInRectangle(rt, Input.mousePosition, Camera.main.transform.parent.GetChild(1).GetComponent<Camera>(), out LocalCursor);
        result.x = Mathf.Clamp( LocalCursor.x, rt.rect.min.x, rt.rect.max.x );
        result.y = Mathf.Clamp(LocalCursor.y, rt.rect.min.y, rt.rect.max.y );
        return rt.rect.Contains(LocalCursor);
    }
    ...
    private void Setup( Color inputColor )
    {
        ...
        Action applySaturationValue = () => {
            ...
            if ( _color != resultColor ) {
                ...
                _color = resultColor;
                Manager.Mat = resultColor;
                Manager.SelectedColorPreview.color = resultColor;
                switch (playerColor)
                {
                    case Colortypes.Primary:
                        CurrentSelected.PrimaryColor = resultColor;
                        break;
                    case Colortypes.Secondary:
                        CurrentSelected.SecondaryColor = resultColor;
                        break;
                    case Colortypes.Details:
                        CurrentSelected.DetailsColor = resultColor;
                        break;
                    case Colortypes.Energy:
                        CurrentSelected.EnergyColor = resultColor;
                        break;
                }
                CurrentSelected.SetColors(CurrentSelected.gameObject);
            }
        };
        applyHue();
